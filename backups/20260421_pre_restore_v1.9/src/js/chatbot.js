...
function initChatbot() {
    ...
    function appendMessage(sender, text) {
        ...
        if (sender === 'user') {
            div.className = 'p-4 rounded-2xl rounded-tr-none bg-primary text-on-primary ml-auto shadow-md text-sm leading-relaxed max-w-[85%] animate-slideUpFade';
        } else {
            div.className = 'p-4 rounded-2xl rounded-tl-none bg-white border border-outline-variant/30 text-on-surface mr-auto shadow-sm text-sm leading-relaxed max-w-[90%] animate-slideUpFade';
        }
...
async function requestGemini(userMessage) {
    // 1. 최적화된 시스템 컨텍스트 생성 (Premium Secretary Personality)
    let systemContext = `너는 대한민국 최고의 전세버스 운송 기업의 '수석 실무 비서'이자 '안전 관제 AI'야.
    너의 임무는 기사님들의 안전을 보호하고, 사내 규정에 근거해 정확한 실무 정보를 제공하는 것이야.
    답변은 항상 정중하고 기사님들을 존중하는 태도로 하되, 전문성을 잃지 마.
    
    [핵심 지침]
    1. 반드시 아래 제공된 [매뉴얼 데이터]에 근거해 답변할 것.
    2. 매뉴얼에 없는 내용은 추측하지 말고 "해당 내용은 현재 시스템 매뉴얼에 등록되어 있지 않습니다. 관리자에게 문의해 주세요."라고 안내할 것.
    3. 중요 수칙(안전, 과로 방지 등)에 대해서는 강조 표시(볼드체)를 사용할 것.
    ...
    if (window.MANUALS_DATABASE) {
        window.MANUALS_DATABASE.forEach(m => {
            ...
            m.sections.forEach(s => {
                systemContext += `- [${s.subtitle}]: ${s.content}\n`;
            });
        });
    }
